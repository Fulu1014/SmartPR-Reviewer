...
🔄 核心工作流
接收推送：接收 GitHub 触发的 pull_request Webhook。

安全核验：对比 X-Hub-Signature-256 签名头。

获取令牌：利用 JWT 向 GitHub API 请求有效期 1 小时的临时 Token。

代码提取：调用 API 获取 PR 涉及的所有文件差异，清洗并拼接代码。

模型审查：将代码发送至大语言模型进行漏洞或逻辑评估。

结果回传：将评估结果以 Issue Comment 形式推送到 GitHub 仓库。
